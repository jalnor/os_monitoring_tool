...
to change width of column drag boundary
    ...
    def open_in_new_tab(self, event):
        ...
        f2 = ttk.Frame(self.parent, width=1000, height=450)
        f2.grid(column=0, row=0, sticky='nsew')
        self.parent.add(f2, text="Process")
        self.on_change_tab(f2)
        process_container = ttk.Frame(f2, width=780, height=768, relief='sunken')
        ...
        ttk.Label(process_container, text="Current Process", padding=2).grid(column=2, row=0)
        ttk.Label(process_container, text="Process History", padding=2).grid(column=3, row=0)

        # Column 1 frame with current process name and id
        name_frame = ttk.Frame(process_container, width=350, height=150,
                               padding=5, borderwidth=5, relief='flat')
        name_frame.grid(column=1, columnspan=2, row=1, rowspan=6, sticky='ne')

        ttk.Label(name_frame, text=selection[0][0]).grid(column=2, row=1)
        ttk.Label(name_frame, text=selection[0][1]).grid(column=2, row=2)

        # Create the data frame and tree to display logs
        data_frame = ttk.Frame(process_container, width=600, height=350, padding=5, borderwidth=5, relief='flat')
        data_frame.grid(column=3, columnspan=5, row=1, sticky='nes')
        ...
        for data in data_for_process:
            ...
            self.adjust_column_width(constant_secondary_tab_headers(), data)
